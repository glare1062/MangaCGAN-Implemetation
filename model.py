...
class Generator(nn.Module):
    ...
    def forward(self, x, y1, y2):
        y1 = func.leaky_relu(self.fc_embed1(y1.squeeze()), 0.2, True)
        y2 = func.leaky_relu(self.fc_embed2(y2.squeeze()), 0.2, True)

        x = torch.cat((x, y1.view(-1, 128, 1, 1), y2.view(-1, 128, 1, 1)), dim=1)

        x = func.leaky_relu(self.bn1(self.tconv1(x)), 0.2, True)

        x = func.leaky_relu(self.bn2(self.tconv2(x)), 0.2, True)
        x = func.leaky_relu(self.bn3(self.tconv3(x)), 0.2, True)
        x = func.leaky_relu(self.bn4(self.tconv4(x)), 0.2, True)

        x = func.tanh(self.tconv5(x))
...
class Discriminator(nn.Module):
    ...
    def forward(self, x, y1, y2):
        ...
        x = func.leaky_relu(self.conv1(x), 0.2, True)
        x = func.leaky_relu(self.bn2(self.conv2(x)), 0.2, True)
        x = func.leaky_relu(self.bn3(self.conv3(x)), 0.2, True)
        x = func.leaky_relu(self.bn4(self.conv4(x)), 0.2, True)
        ...
        x = func.leaky_relu(self.bn5(self.conv5(x)), 0.2, True)
        x = func.sigmoid(self.conv6(x))

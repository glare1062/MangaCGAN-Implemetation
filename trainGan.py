...
manga_dataset = MangaCharDataSet(csv_file='data/clean_labels.csv',
                                 root_dir='data/faces/',
                                 param_file='data/animegan_params.json',
                                 transform=transform)

dataloader = DataLoader(manga_dataset, batch_size=params['bsize'],
                        shuffle=True)
...
onehot = onehot.scatter_(1, torch.LongTensor([i for i in range(params['vocab_size'])]).view(params['vocab_size'], 1),
...
for epoch in range(params['nepochs']):
    for i, data in enumerate(dataloader, 0):
        ...
        if (True):
            ...
            # Gradients for back propagation are calculated.
    ...
    if epoch % params['save_epoch'] == 0:
        torch.save({
            ...
            'params': params
        }, 'checkpoint/model_epoch_{}01.pth'.format(epoch))
        plt.figure(figsize=(6, 6))
        plt.axis("off")
        ...
        with torch.no_grad():
            fake_data = netG(fixed_noise, fixed_condition_ohe1, fixed_condition_ohe2).detach().cpu()
        img_save = plt.imshow(
            np.transpose(vutils.make_grid(fake_data, nrow=6, padding=2, normalize=True).cpu(), (1, 2, 0)))
        img_save.figure.savefig('output/Epoch_{}'.format(epoch))
...
img_save.figure.savefig('output/Epoch_final')
...
# plt.show()
anim.save('output/anime.gif', dpi=100, writer='imagemagick')